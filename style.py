...
def get_model():
    ...
    for key, value in model_dict_clone.items():
        if key.endswith(('running_mean', 'running_var')):
            del model_dict[key]
    res = style_model.load_state_dict(model_dict, False)
    print(res)
    return style_model


def process_style(model, uid, content, style):
    content_image = tensor_load_rgbimage(content, size=512,
    ...
    image = tensor_save_bgrimage(output.data[0], f"{uid}_output.jpg")
    return image